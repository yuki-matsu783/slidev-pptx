...
- **失うもの**: 定義どおりでも一致しない箇所が残る。0 割りと `sqrt` の負数は評価器が 0 に寄せる近似で、PowerPoint の挙動は確かめていない。矢じりは値ごとに PowerPoint と同じ種類の形（`arrow` は開いた V 字、`stealth` は切り欠きのある塗り、など）で描くが、大きさは PowerPoint の「中」の近似（線幅のおよそ 3 倍、下限 6 px）。文字の枠は `w` と `h` がどちらも props にあるときだけ定義どおりに置き、どちらかが無ければ Slidev では枠全体に置く（枠の大きさが文字の量で決まると、文字の枠の余白と循環して測り直しが止まらないため）。そのため `h` だけ書いた図形は、PowerPoint で開くと文字の位置が少しずれる。`cloudCallout` の泡を雲に重ねたときの描き分けも既知の差（ppt-components.md §1.3）。
